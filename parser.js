...
cron.schedule('0 * * * *', function() {
	...
	var request = http.get(fileurl, function(response) {
		response.pipe(file).on('finish', function() {
			shell.exec("iconv "+downloadedCsv+" -f latin1 -t utf-8 -o "+csvFilePath, {silent: true}, function(code, stdout, stderr) {
				if( code == 0 )
				{
					csv({
						noheader: true,
						...
						if( index < 3) return;						
						if( obj.field3 == undefined || obj.field13 == undefined ) return;
